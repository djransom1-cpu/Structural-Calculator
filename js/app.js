...
class StructuralApp {
  ...
  runSteelColumn() {
    ...
    const inputs = {
      ...
      ab_dia: parseFloat(document.getElementById('ab-dia').value) || 0.75,
      ab_grade: parseInt(document.getElementById('ab-grade').value) || 36,
      ab_embed: parseFloat(document.getElementById('ab-embed').value) || 12.0,
      section
    };
    ...
    const metrics = [
      ...
      { label: "Anchor Rods & Embedment", val: `(${res.ab_qty}) ${res.ab_dia}" \u00D8 F1554 Gr ${res.ab_grade} @ ${res.ab_embed_in}" h_ef`, sub: `Tension Allow: ${res.P_rod_tension_allow_kips.toFixed(2)}k (Steel: ${res.P_steel_tension_allow_kips.toFixed(2)}k | Breakout: ${res.P_breakout_allow_kips.toFixed(2)}k) | Req Embedment: ${res.req_hef_in.toFixed(1)}" (${res.passAnchorEmbedment ? 'Pass' : 'FAIL'})` }
    ];

    if (res.isNetTension) {
      metrics.unshift({ label: "🌪️ Net Wind Uplift Tension", val: `${res.P_net_tension_kips.toFixed(1)} kips total`, sub: `${res.Tension_per_rod_kips.toFixed(2)} kips/rod vs ${res.P_rod_tension_allow_kips.toFixed(2)}k allowable (${res.passAnchorTension && res.passAnchorEmbedment ? 'Pass' : 'FAIL'})` });
    }
